...
func testFinder() loader.Finder {
	return func(_ loader.Loader, key string) interface{} {
		return `the ` + key
	}
}

func testNamespace() loader.NsCreator {
	return func(l loader.Loader, key string) loader.Loader {
...
func TestMapLoader_AbsoluteName(t *testing.T) {
	l := loader.New(nil, `my`, vf.Map(`a`, `the a`), nil, func(l loader.Loader, key string) loader.Loader {
...
func TestLoader_Type(t *testing.T) {
	l := loader.New(nil, ``, nil,
		func(l loader.Loader, key string) interface{} {
			return `the ` + key
		},
		func(l loader.Loader, name string) loader.Loader {
...
func TestLoader_Get_notString(t *testing.T) {
	l := loader.New(nil, ``, nil, func(l loader.Loader, key string) interface{} { return key }, nil)
...
func TestLoader_loadRedefine(t *testing.T) {
	first := true
	l := loader.New(nil, ``, nil, func(l loader.Loader, key string) interface{} {
		if first {
			first = false
...
func TestLoader_Namespace(t *testing.T) {
	...
	require.Equal(t, b.Namespace(`b`).Get(`x`), `the x`)
	require.Equal(t, c.Namespace(`a`).Get(`x`), `the x`)
	require.Equal(t, d.Namespace(`a`).Get(`x`), `the x`)
...
func TestLoader_Namespace_noParentNs(t *testing.T) {
	l := loader.New(nil, ``, nil, testFinder(), nil)
	b := l.NewChild(testFinder(), testNamespace())
	require.Nil(t, l.Namespace(`b`))
	require.Equal(t, b.Namespace(`b`).Get(`x`), `the x`)
}

func TestLoader_Namespace_redefinedOk(t *testing.T) {
	first := true
	l := loader.New(nil, ``, nil, nil, func(ld loader.Loader, name string) loader.Loader {
		if first {
			first = false
