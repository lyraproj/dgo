...
func TestFloat(t *testing.T) {
	...
	require.Assignable(t, typ.Float, tf.Float(3.1, 5.8, true))
	require.Assignable(t, typ.Float, vf.Float(4.2).Type())
	require.Equal(t, typ.Float, typ.Float)
	require.Equal(t, typ.Float, tf.Float(-math.MaxFloat64, math.MaxFloat64, true))
...
func TestFloatExact(t *testing.T) {
	...
	require.Assignable(t, tf.Float(3.0, 5.0, true), tp)
	require.Assignable(t, tp, tf.Float(3.1415927, 3.1415927, true))
	require.NotAssignable(t, tp, typ.Float)
	require.Equal(t, tp, tf.Float(3.1415927, 3.1415927, true))
	require.NotEqual(t, tp, tf.Float(2.1, 5.5, true))
...
func TestFloatRange(t *testing.T) {
	tp := tf.Float(3.1, 5.8, true)
	require.Instance(t, tp, 3.1415927)
	require.NotInstance(t, tp, 3.01)
	require.NotInstance(t, tp, true)
	require.Assignable(t, tp, tf.Float(3.1, 5.8, true))
	require.Assignable(t, tp, tf.Float(4.2, 4.2, true))
	require.Assignable(t, tp, vf.Float(4.2).Type())
	require.NotAssignable(t, tp, tf.Float(2.5, 5.5, true))
	require.NotAssignable(t, tp, tf.Float(3.1, 6.2, true))
	require.NotAssignable(t, tp, vf.Float(6.0).Type())
	require.NotAssignable(t, tp, typ.Float)
	require.Equal(t, tp, tf.Float(5.8, 3.1, true))
	require.NotEqual(t, tp, tf.Float(2.5, 5.5, true))
	require.NotEqual(t, tp, typ.Float)
	require.Equal(t, tp.Min(), 3.1)
	require.Equal(t, tp.Max(), 5.8)
	require.Equal(t, vf.Float(4.2).Type(), tf.Float(4.2, 4.2, true))
	...
	tp = tf.Float(3.1, 5.8, false)
	require.Instance(t, tp, 3.1415927)
	require.NotInstance(t, tp, 5.8)
	require.Assignable(t, tp, tf.Float(3.1, 5.8, false))
	require.NotAssignable(t, tp, tf.Float(3.1, 5.8, true))
	require.Assignable(t, tf.Float(3.1, 5.8, true), tp)
	require.Equal(t, `3.1...5.8`, tp.String())

	require.Panic(t, func() { tf.Float(3.1, 3.1, false) }, `cannot have equal min and max`)
...
func TestFloatType_New(t *testing.T) {
	...
	require.Panic(t, func() { vf.New(tf.Float(1, 4, true), vf.Float(5)) }, `cannot be assigned`)
}
